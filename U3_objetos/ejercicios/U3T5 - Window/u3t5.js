...
function crear_ventana() {
  let opcion = confirm("¿Deseas que abra una ventana nueva?");

  if (opcion) {
    ventana = open("", "", "width=200,height=100,top=500,left=500,menubar=no,resizable=no,toolbar=no,location=no");
    ventana.document.write(
      '<p>La ventana de Chema</p></br></br></br></br><button type="button" onclick="window.close()">Cerrar ventana</button>'
    );
  }
}
